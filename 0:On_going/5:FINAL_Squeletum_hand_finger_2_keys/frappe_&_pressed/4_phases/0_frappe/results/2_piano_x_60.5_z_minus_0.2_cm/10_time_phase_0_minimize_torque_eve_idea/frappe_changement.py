...
def custom_func_track_principal_finger_and_finger5_above_bed_key(all_pn: PenaltyNodeList, marker: str) -> MX:
    ...
    markers = BiorbdInterface.mx_to_cx("markers", all_pn.nlp.model.markers, all_pn.nlp.states["q"])
    finger_marker = markers[:, finger_marker_idx]
    markers_diff_key3 = finger_marker[2] - (0.07808863830566405-0.01-0.08)
    return markers_diff_key3


def custom_func_track_principal_finger_and_finger5_behind_front_key(all_pn: PenaltyNodeList, marker: str) -> MX:
    finger_marker_idx = biorbd.marker_index(all_pn.nlp.model, marker)
    markers = BiorbdInterface.mx_to_cx("markers", all_pn.nlp.model.markers, all_pn.nlp.states["q"])
    finger_marker = markers[:, finger_marker_idx]
    markers_diff_key3 = finger_marker[0] - (0.5056114196777344+0.1+0.04)
    return markers_diff_key3
...
def prepare_ocp(
    ...
    constraints.add(custom_func_track_principal_finger_and_finger5_behind_front_key,
                    node=Node.ALL, marker="finger_marker", min_bound=-10000, max_bound=0, phase=0)
    constraints.add(custom_func_track_principal_finger_and_finger5_behind_front_key,
                    node=Node.ALL, marker="finger_marker", min_bound=-10000, max_bound=0, phase=1)
    constraints.add(custom_func_track_principal_finger_and_finger5_behind_front_key,
    ...
    constraints.add(custom_func_track_principal_finger_and_finger5_behind_front_key,
                    node=Node.ALL, marker="finger_marker_5", min_bound=-10000, max_bound=0, phase=0)
    constraints.add(custom_func_track_principal_finger_and_finger5_behind_front_key,
                    node=Node.ALL, marker="finger_marker_5", min_bound=-10000, max_bound=0, phase=1)
    constraints.add(custom_func_track_principal_finger_and_finger5_behind_front_key,
                    node=Node.ALL, marker="finger_marker_5", min_bound=-10000, max_bound=0, phase=2)
    constraints.add(custom_func_track_principal_finger_and_finger5_behind_front_key,
                    node=Node.ALL, marker="finger_marker_5", min_bound=-10000, max_bound=0, phase=3)
    ...
    constraints.add(ConstraintFcn.TIME_CONSTRAINT, phase=0, min_bound=0.1, max_bound=0.5)
...
def main():
    ...
    with open(
            "/0:On_going/5:FINAL_Squeletum_hand_finger_2_keys/frappe_&_pressed/4_phases/0_frappe/results/8_piano_recule_pi_in_two_100000/test10_time_0_minimize_torque_1.pckl", "wb") as file:
