...
with open(
        "/Mathilde_2022/2__FINAL_MODELS_OSCAR/5___final___squeletum_hand_finger_1_key_4_phases/strucked/1_every_dof_minimized_at_100/1_every_dof_minimized_at_100.pckl", 'rb') as file: new_dict = pickle.load(file)
with open(
        "/Mathilde_2022/2__FINAL_MODELS_OSCAR/5___final___squeletum_hand_finger_1_key_4_phases/strucked/2_finger_hand_ulna_radius_minimize_at_10_000/4_a_finger_hand_radius_ulna_10_000_&_100.pckl", 'rb') as file: new_dict2 = pickle.load(file)
...
# COMMUN ##################################################
T = np.hstack((np.linspace(0, 0.3, num=94), np.linspace(0.3, 0.3+0.027, num=94), np.linspace(0.3+0.027, 0.3+0.027+0.058, num=94), np.linspace(0.3+0.027+0.058, 0.3+0.027+0.058+0.3, num=94)))
...
figQ.suptitle('States (q) of limbs by minimizing more the finger, hand, radius & ulna for a staccato strucked attack of one key.', fontweight="bold", size=18)
...
figQdot.suptitle('States (qdot) of limbs by minimizing more the finger, hand, radius & ulna for a staccato strucked attack of one key.', fontweight="bold", size=18)
...
figU.suptitle('Torque (tau) of limbs by minimizing more the finger, hand, radius & ulna for a staccato strucked attack of one key.', fontweight="bold", size=18)
